...
- **Spend.** A pay-as-you-go budget (`usage.spend`) renders as its own meter and gauge, tinted red at 100%. It is **display-only** — a maxed spend budget never marks an account rate-limited or removes it from rotation. Accounts with no such plan show nothing (that is "no plan", not "unknown"). On a large pool the spend figure folds into the account label instead, to stay inside the host's node budget for the panel; the switch is automatic and depends on how many accounts and per-model windows you have. (It is deliberately conservative — it can fold spend into the label slightly earlier than strictly necessary, never later.)
